...
import { submitToLOA, qualifyLead, CreativeProjectData } from '../services/loaApi';

interface CreativeProjectPageProps {
  selectedPlan?: string;
  onNavigate: (page: string) => void;
}

export function CreativeProjectPage({ selectedPlan, onNavigate }: CreativeProjectPageProps) {
  ...
  const plans = [
    { name: 'EventOS Basic Boost', price: '$1,500 CAD', value: 'basic', description: 'EventOS Platform License with basic features + Basic Design Package' },
    { name: 'EventOS Standard Pro', price: '$3,000 CAD', value: 'standard', description: 'EventOS + AI Event Operator + Analytics + Pro Design Package' },
    { name: 'EventOS Premium Elite', price: '$5,000 CAD', value: 'premium', description: 'EventOS + AI + White-label Rights + Elite Design Package' },
    { name: 'EventOS Custom Scale', price: 'Custom Pricing', value: 'custom', description: 'Enterprise solutions with custom features + Custom Design Package' },
  ];

  const projectTypes = [
    'EventOS Platform + Web Design',
    'EventOS Platform + Brand Identity',
    'EventOS Platform + E-commerce Development',
    'EventOS Platform + Mobile App',
    'EventOS Platform + Marketing Campaign',
    'EventOS Platform + Full Digital Transformation',
    'Web Design Only',
    'Brand Identity Only',
    'SEO Optimization Only',
    'Custom Creative Solution',
  ];
  ...
  const handleSubmit = async () => {
    try {
      ...
      const loaData: CreativeProjectData = {
        service_type: formData.plan,
        project_name: formData.projectType || `Creative ${formData.plan} Project`,
        ...
        event_type: formData.plan.includes('eventos') ? 'corporate' : undefined,
        expected_attendees: formData.plan.includes('eventos') ? 100 : undefined,
        monetization_strategy: formData.description
      };

      // Send to LOA API
      const result = await submitToLOA(loaData, 'creative');
      
      if (result.success) {
        // Get qualification from LOA
        const qualification = await qualifyLead(loaData, 'creative');
        
        if (qualification) {
          console.log('🎯 Creative Qualification Score:', qualification.qualification.score);
          ...
          const submissions = JSON.parse(localStorage.getItem('creative_project_submissions') || '[]');
          submissions.push({
            ...loaData,
          ...
          localStorage.setItem('creative_project_submissions', JSON.stringify(submissions));
        }
      ...
      <div className="min-h-screen bg-[#1A1A1A] flex items-center justify-center px-4 py-20">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <h1 className="text-4xl sm:text-5xl text-white mb-6">
            <span className="font-futuristic">Creative Project</span>{' '}
            <span className="font-graffiti text-[#FF7A00]">Submitted!</span>
          </h1>
          <p className="text-gray-400 text-lg mb-4">
            Thank you for choosing 9LMNTS Studio! We've received your creative project details.
          </p>
          ...
          <div className="bg-[#222222] border border-[#FF7A00]/20 rounded-lg p-8 mb-8">
            ...
            <div className="space-y-4 text-left">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-white mb-1">Project Review</h4>
                  <p className="text-gray-400 text-sm">We'll analyze your EventOS and creative requirements and prepare a tailored proposal</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 bg-[#FF7A00] rounded-full flex items-center justify-center flex-shrink-0 mt-1">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-white mb-1">Project Kick-Off</h4>
                  <p className="text-gray-400 text-sm">Start bringing your vision to life with 9LMNTS process</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl sm:text-5xl text-white mb-4">
            ...
            <span className="font-graffiti text-[#FF7A00]">Creative Project</span>
          </h1>
          <p className="text-gray-400 text-lg">
            Let's bring your creative vision to life in {4 - step + 1} simple steps
          </p>
        </div>
        ...
        {/* Step 1: Select EventOS + Creative Plan */}
        {step === 1 && (
          <div className="bg-[#222222] rounded-lg p-8 mb-8">
            <h2 className="text-2xl text-white mb-6">Choose Your Creative Solution</h2>
        ...
        {step === 2 && (
          <div className="bg-[#222222] rounded-lg p-8 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-gray-400 mb-2">Creative Service Type</label>
                <select
                ...
                >
                  <option value="">Select Creative Service</option>
            ...
            <div className="mt-6">
              ...
              <textarea
                ...
                placeholder="Describe your EventOS and creative project requirements..."
                rows={4}
                className="w-full px-4 py-3 bg-[#1A1A1A] border border-[#333333] rounded-lg text-white focus:border-[#FF7A00] focus:outline-none"
              />
            </div>
        ...
        {step === 4 && (
          <div className="bg-[#222222] rounded-lg p-8 mb-8">
            <h2 className="text-2xl text-white mb-6">Review Your Creative Project</h2>
            <div className="bg-[#1A1A1A] rounded-lg p-6 mb-6">
              <h3 className="text-lg text-white mb-4">Project Summary</h3>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-400">Creative Plan:</span>
            ...
            <div className="bg-[#FF7A00]/10 border border-[#FF7A00]/20 rounded-lg p-4 mb-6">
              <p className="text-[#FF7A00] font-bold">🚀 Ready to Create?</p>
              <p className="text-white text-sm mt-2">Click submit to start your EventOS + Creative project with 9LMNTS Studio</p>
            </div>
            <button
